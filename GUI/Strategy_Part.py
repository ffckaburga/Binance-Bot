...
class StrategyManager(tk.Frame):
    def __init__(self, root: "Root", binance: BinanceClient, *args, **kwargs):
        ...
        self._valid_integer = self.register(is_correct_integer_format)
        self._valid_float = self.register(is_correct_float_format)
        ...
        for symbol, contract in self.binance.contracts.items():
        ...
        self._add_button = tk.Button(self._commands_frame, text="Add strategy", font=GLOBAL_FONT,
                                     command=self._add_strategy_row, bg=BACKGROUND_2, fg=FOREGROUND,highlightthickness=1,highlightbackground = "white")
        ...
        self._base_params = [
            ...
            {"code_name": "parameters", "widget": tk.Button, "data_type": float, "text": "Parameters",
             "bg": BACKGROUND_2, "command": self._show_popup, "header": "", "width": 10},
        ...
        for idx, h in enumerate(self._base_params):
            header = tk.Label(self._headers_frame, text=h['header'], bg=BACKGROUND, fg=FOREGROUND, font=GLOBAL_FONT,
                              width=h['width'], bd=1, relief=tk.FLAT)
            header.grid(row=0, column=idx, padx=2)

        header = tk.Label(self._headers_frame, text="", bg=BACKGROUND, fg=FOREGROUND, font=GLOBAL_FONT,
        ...
        self._body_frame = ScrollableFrame(self._table_frame, bg=BACKGROUND, height=230)
        ...
        for col, base_param in enumerate(self._base_params):
            ...
            if base_param['widget'] == tk.OptionMenu:
                ...
                self.body_widgets[code_name][b_index] = tk.Button(self._body_frame.sub_frame, text=base_param['text'],
                                        bg=base_param['bg'], fg=FOREGROUND, font=GLOBAL_FONT, width=base_param['width'],
                                        command=lambda frozen_command=base_param['command']: frozen_command(b_index))
            else:
                continue

            self.body_widgets[code_name][b_index].grid(row=b_index, column=col, padx=2,pady=3)
    ...
    def _show_popup(self, b_index: int):
        ...
        self._popup_window.wm_title("Parameters for the Strategy")

        self._popup_window.config(bg=BACKGROUND)
        ...
        for param in self.extra_params[strat_selected]:
            ...
            temp_label = tk.Label(self._popup_window, bg=BACKGROUND, fg=FOREGROUND, text=param['name'], font=BOLD_FONT)
            temp_label.grid(row=row_nb, column=0)

            if param['widget'] == tk.Entry:
                self._extra_input[code_name] = tk.Entry(self._popup_window, bg=BACKGROUND_2, justify=tk.CENTER, fg=FOREGROUND,
                                                        insertbackground=FOREGROUND, highlightthickness=False)
        ...
        validation_button = tk.Button(self._popup_window, text="Validate", bg=BACKGROUND_2, fg=FOREGROUND,
    ...
    def _switch_strategy(self, b_index: int):
        ...
        if self.body_widgets['activation'][b_index].cget("text") == "OFF":
            ...
            if strat_selected == "Technical":
                new_strategy = TechnicalStrategy(self.client, contract, timeframe, balance_pct,
                                                 take_profit, stop_loss, self.additional_parameters[b_index])
            elif strat_selected == "Breakout":
                new_strategy = BreakoutStrategy(self.client, contract, timeframe, balance_pct,
                                                take_profit, stop_loss, self.additional_parameters[b_index])
            else:
                return
            ...
            self.binance.subscribe_channel([contract], "bookTicker")
