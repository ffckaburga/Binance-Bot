...
from GUI.Log_Part import Logging
from GUI.Watchlist_Part import Watchlist
from GUI.Trades_Part import TradesWatch
from GUI.Strategy_Part import StrategyManager
...
class Root(tk.Tk):
    def __init__(self, binance: BinanceClient):
        ...
        self._left_frame = tk.Frame(self, bg=BACKGROUND)
        self._left_frame.pack(side=tk.LEFT)

        self._right_frame = tk.Frame(self, bg=BACKGROUND)
        self._right_frame.pack(side=tk.LEFT)

        # Creates and places components at the top and bottom of the left and right frame

        self._watchlist_frame = Watchlist(self.binance.contracts, self._left_frame, bg=BACKGROUND)
        self._watchlist_frame.pack(side=tk.TOP, padx=10)

        self.logging_frame = Logging(self._left_frame, bg=BACKGROUND)
        self.logging_frame.pack(side=tk.TOP, pady=15)  # Space a bit the components with vertical padding

        self._strategy_frame = StrategyManager(self, self.binance, self._right_frame, bg=BACKGROUND)
        self._strategy_frame.pack(side=tk.TOP, pady=15)

        self._trades_frame = TradesWatch(self._right_frame, bg=BACKGROUND)
    ...
    def _ask_before_close(self):
        ...
        if result == "yes":
            self.binance.reconnected = False  # Avoids the infinite reconnect loop in _start_ws()
